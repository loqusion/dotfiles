abbr d docker
abbr db 'docker image build'
abbr dbt 'docker image build -t'
abbr dcn 'docker container'
abbr dhi 'docker image history'
abbr di 'docker image'
abbr dlo 'docker container logs'
abbr dls 'docker image ls'
abbr dpl 'docker image pull'
abbr dprb 'docker builder prune'
abbr dprc 'docker container prune'
abbr dpri 'docker image prune'
abbr dprs 'docker system prune'
abbr dprv 'docker volume prune'
abbr dps 'docker container ps'
abbr dpsa 'docker container ps --all'
abbr dpu 'docker image push'
abbr dr 'docker container run'
abbr drm 'docker container rm'
abbr drm! 'docker container rm --force'
abbr drmi 'docker image rm'
abbr ds 'docker container start'
abbr dst 'docker container stop'
abbr dt 'docker image tag'
abbr dx 'docker container exec -it'

abbr dn 'docker network'

abbr dc 'docker compose'
abbr dcb 'docker compose build'
abbr dcc --set-cursor 'docker compose config % | bat -l yaml'
abbr dcd 'docker compose down'
abbr dcf 'docker compose -f'
abbr dcl 'docker compose logs'
abbr dclF 'docker compose logs --follow --tail 0'
abbr dclf 'docker compose logs --follow'
abbr dcls 'docker compose ls'
abbr dcpl 'docker compose pull'
abbr dcps 'docker compose ps'
abbr dcr 'docker compose run'
abbr dcu 'docker compose up'
abbr dcud 'docker compose up --detach'
abbr dcw 'docker compose up --watch'
abbr dcx 'docker compose exec'
